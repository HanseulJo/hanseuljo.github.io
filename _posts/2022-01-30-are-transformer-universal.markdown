...
$\R^{d\times n}$에서 $\R^{d\times n}$로 가는 함수를 **sequence-to-sequence** function이라고 말한다. 정확히는 정의역도 치역도 모두 subset of $\R^{d\times n}$인 함수를 말한다. ($\R^{d\times n}$: the set of all $d\times n$  real matrices)
...
- Sequence-to-sequence 함수의 연속성 정의
    ...
    Sequence-to-sequence function이 행렬을 받아 행렬을 내뱉는 함수이다 보니 연속성도 잘 정의되어야 한다. 논문에서는 $\R^{d\times n}$에 entry-wise $\ell^p$ norm($\|\cdot\|_p$)과 그에 대한 [norm topology](https://mathworld.wolfram.com/NormTopology.html)를 주고 그 위에서 연속성을 정의하는 것으로 보인다. 이때 $p$의 값은 $1\le p<\infty$. 
    
- 함수 간의 거리(function metric)
    
    함수끼리 얼마나 가까운 지를 나타내기 위해 function 사이의 distance를 정의한다. 즉 sequence-to-sequence function space의 metric을 쓰자면 다음과 같다. 
    
    $$
    N_p(f) := \left(\int_{\R^{d\times n}} \| f(\boldsymbol{X})\|_p^p ~d\boldsymbol{X}\right)^{1/p}; ~\operatorname{d}_p (f_1, f_2 ) := N_p(f_1 - f_2)
    $$
    
    (Usual한 $\ell^p$  function norm을 이용해서, 논문에 있는 표기와 조금 다르게 적어보았다.)
...
- Permutation matrix란
    ...
    Permutation matrix는 각 행과 각 열마다 1이 딱 하나씩 있는 정사각행렬이다. 어떤 행렬 $A\in \R^{m\times n}$에 Permutation matrix $P$를 곱하면 $A$의 행 또는 열의 순서를 뒤죽박죽 섞어 놓은 것과 같다. 좀 더 정확히는, (1) $P\in \R^{n\times n}$이라면 $AP$는 $A$의  열들의 순서를 섞어놓은 행렬이 되고, (2) $P\in \R^{m\times m}$이라면 $PA$는 $A$의  행들의 순서를 섞어놓은 행렬이 된다. 예를 들자면 다음과 같다.
    
    $$
    \footnotesize \begin{pmatrix} 1&2&3 \\ 4&5&6 \\ 7&8&9\end{pmatrix}\begin{pmatrix} 0&1&0 \\ 0&0&1 \\ 1&0&0\end{pmatrix} = \begin{pmatrix} 3&1&2 \\ 6&4&5 \\ 9&7&8\end{pmatrix}
    $$
    
    참고로 이러한 permutation matrix는 언제나 orthogonal하다: $P^TP=PP^T=I$. (P가 행/열의 순서를 어떻게 섞는지 생각해보자.)
    

임의의 $X\in \R^{m\times n}$와 임의의 permutation matrix $P\in \R^{n\times n}$에 대해서, Sequence-to-sequence function인 $f$가 $f(XP)=f(X)P$를 만족하면 이러한 함수가 permutation equivariant하다고 말한다.
...
![transformer-formula](/assets/img/trasformer-formula.jpeg)
...
![contextual-mapping.jpeg](/assets/img/contextual-mapping.jpeg)
...
$$
\footnotesize \boldsymbol{E} = \begin{pmatrix} 0&1&2&\cdots&n-1\\0&1&2&\cdots&n-1\\\vdots&\vdots&\vdots&&\vdots\\0&1&2&\cdots&n-1\end{pmatrix}
$$
...
<aside>
❓ 논문에서는 증명을 위해 아주 작은 transformer block을 이용하고 있다. 만약 이 transformer block의 크기를 키운다면 필요한 block의 수는 줄어들까? (아마 d와 n에 따른 complexity에는 크게 차이가 있지 않을 것 같다. h, m, r 등의 값은 d나 n의 값과는 관련이 없으므로.)
...
- 위에서 던졌던 질문들은 내가 논문을 읽으면서도 끝까지 이해하지 못했던, 혹은 스스로 만족스럽게 대답하지 못했던 대표적인 질문들이다. 한 번 더 모아보자면 아래와 같다.
